...
subroutine set_camp_gas_state( mam4,camp_state,chem_spec_data )
    ...
    do i = 1_i_kind, size( camp_state%state_var )
        ...
        if ( j > 0 ) then
            print *, 'Found name ', chem_spec_data%gas_state_name( i ), ' at index ', j
    ...
    select type ( aero_rep_data )
        type is ( aero_rep_modal_binned_mass_t )
            ...
            do i = 1_i_kind, size( names )
                ...
                camp_state%state_var( id ) = mam4( j ) * aircon * ( 1e-3_r8 * adv_mass( j ) )
            end do
    end select
    deallocate( names )
    ...
    select type ( aero_rep_data )
        type is ( aero_rep_modal_binned_mass_t )
            ...
            do i = 1_i_kind, size( names )
                ...
                mam4( j ) = camp_state%state_var( id ) / ( 1e-3_r8 * adv_mass( j ) ) / aircon
            end do
    end select
    deallocate( names )
